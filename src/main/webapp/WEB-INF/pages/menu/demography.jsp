        ...
        <table class="ranking">
            ...
            <c:forEach items="${cities}" var="city">
                <tr style="height: 35px;">
                    ...
                    <td><span style="vertical-align:middle;">${city.population} </span><c:if test="${city.satisfaction <= 30 && city.population > 10}"><img style="vertical-align:middle;" src="assets/negIndicator.png" /></c:if><c:if test="${city.satisfaction >= 80 && city.population < city.countBuildings(1) * 10 + 10}"><img style="vertical-align:middle;" src="assets/posIndicator.png" /></c:if></td>
                    <td><span class="<c:if test="${city.satisfaction <= 30}">red</c:if><c:if test="${city.satisfaction >= 80}">green</c:if>">${city.satisfaction}</span><c:if test="${city.satisfaction > city.calculatedSatisfaction && city.type.id < 4}"><img style="vertical-align:middle;" src="assets/negIndicator.png" /></c:if><c:if test="${city.satisfaction < city.calculatedSatisfaction && city.type.id < 4}"><img style="vertical-align:middle;" src="assets/posIndicator.png" /></c:if></td>
                    ...
                    <td><button title="Baumenü öffnen" style="height:36px; margin-right:2px;" onclick="openMenu('build?city=${city.id}&demography=1')"><img style="vertical-align:middle; width:30px; height:30px;" src="assets/build.png" /></button><button title="Alias für Stadt benennen" style="height:36px;" onclick="setAlias('${city.id}', '${city.name}', '${city.alias}')"><img style="vertical-align:middle;" src="assets/setAlias.png" /></button></td>
                </tr>
            </c:forEach>
            </tbody>
            </c:if>
