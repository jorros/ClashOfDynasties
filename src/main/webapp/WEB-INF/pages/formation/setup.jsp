...
<script>
    function save()
    {
        ...
        $.get("/game/formations/save", { "formation": "${formation.id}", "city": "${city.id}", "units": assigned, "name": $("#formation_name").val() });
        closeMenu();
    }
</script>
            ...
            <div style="height:270px; overflow-y:auto; overflow-x:hidden;">
                <ul id="city_list" class="sortable">
                    <c:forEach items="${city.units}" var="unit">
                        <li id="unit_${unit.id}"><img src="/game/formations/health?health=${unit.health}&unit=${unit.blueprint.id}" /></li>
                    </c:forEach>
                </ul>
            </div>
        </div>
